...
/// Controller for the Confirm & Edit Segment Route screen.
class ConfirmYourRouteSegmentController extends GetxController {
  // ─────────────────────────────────────────────────────────────
  // CONSTANTS
  // ─────────────────────────────────────────────────────────────
  ...
  void _initializeFromArguments() {
    try {
      ...
      debugPrint('ConfirmYourRouteSegmentController init error: $e');
      _setDefaultWaypoints();
    }
  }
  ...
  Future<void> fetchPermitDetails(String routeId, String permitId) async {
    try {
      ...
      debugPrint(
          '🌐 [FetchPermitSegment] Fetching permits → routeId=$routeId, permitId=$permitId');
      ...
      if (permitItem == null) {
        debugPrint(
            '❌ [FetchPermitSegment] Permit #$permitId not found in response list.');
        Get.snackbar(
          'Error',
      ...
      debugPrint(
          '✅ [FetchPermitSegment] Found permit: ${permitItem.displayTitle} — ${permitItem.waypoints.length} waypoints');
      ...
      debugPrint('❌ [FetchPermitSegment] Error: $e');
      Get.snackbar(
        'Error',
