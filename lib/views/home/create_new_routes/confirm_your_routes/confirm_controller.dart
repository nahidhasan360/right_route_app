...
// ///
// /// Responsibilities:
// ///  - Parse route data from navigation arguments
// ///  - Manage the MapLibre map (markers, real-road polyline via OSRM)
// ///  - Expose observables that the view binds to
// ///  - Handle local GPS for "Your current location" waypoints
...
//
//   /// Exposed so the view can read the map-centre address (optional UI use).
//   final RxString currentLocationAddress = 'Fetching location...'.obs;
//
//   // ─────────────────────────────────────────────────────────────
...
//
//   /// Initial camera target – updated once GPS resolves.
...
//
//   // Route-draw concurrency control
//   bool _routeDrawInProgress = false;
//   int _routeGeneration = 0; // incremented on every draw; stale draws exit early
//
//   // ─────────────────────────────────────────────────────────────
//   // LIFECYCLE
//   // ─────────────────────────────────────────────────────────────
...
//
//       // Helper: look up geocoded coordinates by location label
//       LatLng? _coordFor(String label) {
...
//       }
//
//       // Helper: append one waypoint with a fallback offset position
//       void _appendWaypoint(String label, LatLng? coord) {
//         waypoints.add(label);
...
//       _appendWaypoint(startLabel, _coordFor(startLabel));
//
//       // Middle segments
//       for (final seg in routeSegments) {
//         if (seg.isNotEmpty) _appendWaypoint(seg, _coordFor(seg));
//       }
//
//       // End
//       if (endLocation?.isNotEmpty == true) {
//         _appendWaypoint(endLocation!, _coordFor(endLocation!));
//       }
//
//       // Route / permit name
...
//       if (status.isGranted) await _getCurrentLocation();
//     } catch (e) {
...
//
//       // Replace placeholder "Your current location" with resolved GPS
//       if (waypoints.isNotEmpty &&
//           waypoints[0] == 'Your current location' &&
//           _waypointPositions.isNotEmpty) {
//         _waypointPositions[0] = currentLocation;
//         final address = await _reverseGeocode(pos.latitude, pos.longitude);
//         currentLocationAddress.value = address;
//         waypoints[0] = address;
//         if (waypointControllers.isNotEmpty) {
//           waypointControllers[0].text = address;
//         }
//       }
//
...
//
//   // ─────────────────────────────────────────────────────────────
//   // MAP EVENTS  (called from the view's MapLibreMap callbacks)
//   // ─────────────────────────────────────────────────────────────
//   void onMapCreated(MapLibreMapController controller) {
//     mapController = controller;
//     isMapReady.value = true;
//
//     // Symbol tap → select / deselect pin
//     controller.onSymbolTapped.add((symbol) {
//       final idx = _waypointSymbols.indexOf(symbol);
//       if (idx != -1) _handlePinTap(idx);
//     });
//
//     // Pin drag → update position + reverse geocode on release
...
//
//         final address = await _reverseGeocode(current.latitude, current.longitude);
//         if (idx < waypoints.length) waypoints[idx] = address;
//         if (idx < waypointControllers.length) {
//           waypointControllers[idx].text = address;
//         }
//
//         // Redraw markers + real-road route after drag
//         await _refreshMap();
//       }
//     });
//   }
...
//   // ─────────────────────────────────────────────────────────────
//
//   /// Master refresh: redraws markers + real-road polyline + recalculates distance.
...
//     if (mapController == null || !_iconsLoaded) return;
//     try {
//       // Remove all existing waypoint symbols in one batch call
...
//         final isSelected = i < _waypointSelectedStates.length &&
//             _waypointSelectedStates[i];
...
//   }
//
//   /// Draws a real-road polyline via OSRM with retry logic.
//   /// Concurrent calls are serialised: a new call cancels any in-flight draw.
//   Future<void> _drawRealRoadPolyline() async {
//     if (mapController == null || _waypointPositions.length < 2) return;
//
//     // Cancel any previous in-flight draw
//     final generation = ++_routeGeneration;
//     if (_routeDrawInProgress) return; // let the ongoing draw finish naturally
//
//     _routeDrawInProgress = true;
//     isRouteLoading.value = true;
//
//     try {
//       // Remove stale route line
//       if (_routeLine != null) {
//         try {
...
//       }
//
//       if (generation != _routeGeneration) return; // superseded
//
//       final routeGeometry =
...
//   }
//
//   /// Fetches a real-road geometry from OSRM with up to [_osrmMaxRetries] retries.
//   /// Falls back to straight-line waypoints on total failure.
...
//       try {
//         final response =
//         await http.get(uri, headers: {'User-Agent': 'RightRoutes/1.0'})
...
//     }
//
//     debugPrint('OSRM: all retries exhausted – using straight-line fallback');
//     return List.from(points); // straight-line fallback
//   }
//
...
//     }
//     // Only update markers (not full route redraw)
//     _addAllMarkers();
//   }
//
//   void _deselectAll() {
...
//     for (int i = 0; i < _waypointPositions.length; i++) {
//       final d =
//           (tap.latitude - _waypointPositions[i].latitude).abs() +
...
//     final address =
//     await _reverseGeocode(_mapCenter.latitude, _mapCenter.longitude);
//     waypoints.add(address);
//     waypointControllers.add(TextEditingController(text: address));
//     _waypointPositions.add(_mapCenter);
//     _waypointSelectedStates.add(false);
//     await _refreshMap();
//   }
//
...
//     waypointControllers[idx].dispose();
...
//     if (index >= 0 && index < waypoints.length) waypoints[index] = val;
//   }
//
...
//   }
//
//   /// Read-only view of waypoint positions for the GO navigation payload.
...
class ConfirmRouteController extends GetxController {
  ...
  // NEW: State for "Add Pin" interactive mode
